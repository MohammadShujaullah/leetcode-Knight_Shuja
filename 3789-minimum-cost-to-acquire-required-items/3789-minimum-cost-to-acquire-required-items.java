class Solution {
    ...
    public long minimumCost(int cost1, int cost2, int costBoth, int need1, int need2) {
        ...
        long mincostboth=Math.min(costBoth,(long)cost1+cost2);

        long mincost1=Math.min(costBoth,cost1);
        long mincost2=Math.min(costBoth,cost2);
        ...
        long common=Math.min(need1,need2);
         ...
         totalcost+=common*mincostboth;


         if(need1>need2){
            totalcost+=(need1-need2)*mincost1;


         }
         else{
            totalcost+=(need2-need1)*mincost2;
         }
